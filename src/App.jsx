// ============================================================================
// YetkinGIS - Kent Rehberi
// 3 Rol: superadmin | belediye_admin | vatandas
//
// DÜZELTMELER (bu versiyon):
//   - Vatandaş panelinde şifre butonu kaldırıldı (canEdit koşulu)
//   - Çizim aracı geçişlerinde siyah ekran / removeChild hatası giderildi
//     (applyDrawInteraction try-catch + güvenli interaction temizliği)
//   - Harita cleanup'ı güçlendirildi
// ============================================================================
...
function SifreDegistirModal({ kullanici, onKapat }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000c", display:"flex",
      ...
      <div style={{ background:"#1e2030", border:"1px solid #2a2f45", borderRadius:16, padding:28, width:360 }}>
        ...
        {basari && (
          <div style={{ background:"#0f2a1a", border:"1px solid #22c55e44", borderRadius:8,
            padding:"10px 12px", color:"#22c55e", fontSize:13, marginBottom:14 }}>
            ✅ Şifre başarıyla değiştirildi!
          </div>
        )}
        {hata && (
          <div style={{ background:"#2a0f0f", border:"1px solid #ef444444", borderRadius:8,
            padding:"10px 12px", color:"#ef4444", fontSize:13, marginBottom:14 }}>
            ⚠ {hata}
          </div>
        )}
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:8 }}>
          <button onClick={onKapat}
            style={{ padding:"9px 16px", background:"transparent", border:"1px solid #2a2f45",
              borderRadius:8, color:"#7a80a0", cursor:"pointer", fontFamily:"inherit", fontSize:13 }}>
            İptal
          </button>
          <button onClick={handleKaydet}
            style={{ padding:"9px 16px", background:"#3b82f6", border:"none",
              borderRadius:8, color:"#fff", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:600 }}>
            💾 Kaydet
          </button>
        </div>
      </div>
...
// ============================================================================
// KULLANICI YÖNETİM MODALİ (sadece superadmin)
// ============================================================================
function KullaniciYonetimModal({ onKapat }) {
  ...
  function handleEdit(u) {
    setEditId(u.id); setYeniSifre(""); setYeniAd(u.ad); setBasari(""); setHata("");
  }
  ...
  function handleKaydet(u) {
    ...
    const updated = users.map(x => x.id === u.id
      ? { ...x, ad: yeniAd, ...(yeniSifre ? { sifre: yeniSifre } : {}) }
      : x
    );
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000c", display:"flex",
      ...
      <div style={{ background:"#1a1d27", border:"1px solid #a855f744", borderRadius:16, padding:28,
        ...
        <div style={{ fontSize:12, color:"#7a80a0", marginBottom:20 }}>
          Şifre değiştirme ve kullanıcı bilgilerini düzenleme
        </div>

        {basari && (
          <div style={{ background:"#0f2a1a", border:"1px solid #22c55e44", borderRadius:8,
            padding:"8px 12px", color:"#22c55e", fontSize:12, marginBottom:12 }}>✅ {basari}</div>
        )}
        {hata && (
          <div style={{ background:"#2a0f0f", border:"1px solid #ef444444", borderRadius:8,
            padding:"8px 12px", color:"#ef4444", fontSize:12, marginBottom:12 }}>⚠ {hata}</div>
        )}

        {users.map(u => (
          <div key={u.id} style={{ background:"#0f1117", border:`1px solid ${rolRenk[u.rol]}33`,
            borderRadius:10, padding:14, marginBottom:10 }}>
            {editId === u.id ? (
              <>
                <div style={{ fontSize:11, color:rolRenk[u.rol], marginBottom:10, fontWeight:600 }}>
                  ✏ {rolLabel[u.rol]} — {u.email}
                </div>
                <div style={{ marginBottom:8 }}>
                  <div style={{ fontSize:11, color:"#7a80a0", marginBottom:4 }}>Ad Soyad</div>
                  <input value={yeniAd} onChange={e => setYeniAd(e.target.value)}
                    style={{ width:"100%", padding:"7px 10px", background:"#1a1d27", border:"1px solid #2a2f45",
                      borderRadius:6, color:"#e8eaf0", fontSize:12, outline:"none",
                      boxSizing:"border-box", fontFamily:"inherit" }} />
                </div>
                <div style={{ marginBottom:10 }}>
                  <div style={{ fontSize:11, color:"#7a80a0", marginBottom:4 }}>
                    Yeni Şifre <span style={{ color:"#4a5070" }}>(boş bırakırsan değişmez)</span>
                  </div>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  <button onClick={() => { setEditId(null); setHata(""); }}
                    style={{ padding:"6px 12px", background:"transparent", border:"1px solid #2a2f45",
                      borderRadius:6, color:"#7a80a0", cursor:"pointer", fontFamily:"inherit", fontSize:12 }}>
                    İptal
                  </button>
                  <button onClick={() => handleKaydet(u)}
                    style={{ padding:"6px 12px", background:rolRenk[u.rol], border:"none",
                      borderRadius:6, color:"#fff", cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:600 }}>
                    💾 Kaydet
                  </button>
                </div>
              </>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <button onClick={() => handleEdit(u)}
                  style={{ padding:"6px 12px", background:"#21253a", border:`1px solid ${rolRenk[u.rol]}55`,
                    borderRadius:6, color:rolRenk[u.rol], cursor:"pointer", fontFamily:"inherit", fontSize:11 }}>
                  ✏ Düzenle
                </button>
              </div>
            )}
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ width:"100vw", height:"100vh", background:"#0f1117",
      ...
      <div style={{ background:"#1a1d27", border:"1px solid #2a2f45", borderRadius:20, padding:36, width:380 }}>
        ...
        {hata && <div style={{ background:"#ef444422", border:"1px solid #ef444466",
          borderRadius:8, padding:"10px 12px", fontSize:13, color:"#ef4444", marginBottom:14 }}>{hata}</div>}
        ...
        <button style={ls.btnPrimary} onClick={handleLogin} disabled={yukleniyor}>
...
function SikayetFormModal({ konum, onKaydet, onIptal }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000c", display:"flex",
      alignItems:"center", justifyContent:"center", zIndex:400, backdropFilter:"blur(4px)" }}>
      <div style={{ background:"#1e2030", border:`1px solid ${secilenKat?.color}44`,
        borderRadius:16, padding:24, width:420, maxHeight:"80vh", overflowY:"auto",
        boxShadow:`0 0 40px ${secilenKat?.color}22` }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:"#7a80a0" }}>
              📍 {konum ? `${konum[1].toFixed(5)}, ${konum[0].toFixed(5)}` : "Konum seçilmedi"}
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <input style={sf.input} placeholder="Kısa bir başlık girin"
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
          <div>
            ...
            <input style={sf.input} placeholder="İsteğe bağlı"
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          <button onClick={onIptal}
            style={{ padding:"9px 16px", background:"transparent", border:"1px solid #3a3f55",
              borderRadius:8, color:"#7a80a0", cursor:"pointer", fontFamily:"inherit", fontSize:13 }}>
            İptal
          </button>
          <button onClick={() => {
              if (!baslik.trim()) { alert("Başlık zorunludur."); return; }
              onKaydet({ kategori, baslik, aciklama, adSoyad, telefon,
                durum:"beklemede", tarih: new Date().toLocaleString("tr-TR") });
            }}
            style={{ padding:"9px 16px", background: secilenKat?.color || "#3b82f6",
...
const sf = {
  ...
  input: { width:"100%", padding:"8px 10px", background:"#0f1117", border:"1px solid #2a2f45",
    borderRadius:8, color:"#e8eaf0", fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"inherit" },
};

// ============================================================================
// ÖZNITELIK FORMU MODALİ
// ============================================================================
function OzellikFormModal({ ozellikForm, katmanDefs, onKaydet, onIptal }) {
  ...
  const KATMAN_ALANLARI = {
    parsel:    [{ ad:"ada_no", label:"Ada No", tip:"text" }, { ad:"parsel_no", label:"Parsel No", tip:"text" },
                { ad:"alan_m2", label:"Alan (m²)", tip:"number" }, { ad:"malik", label:"Malik", tip:"text" }],
    yapi:      [{ ad:"yapi_adi", label:"Yapı Adı", tip:"text" }, { ad:"kat_sayisi", label:"Kat Sayısı", tip:"number" },
                { ad:"yapi_turu", label:"Yapı Türü", tip:"select", secenekler:["Konut","Ticari","Kamu","Dini","Sanayi","Diğer"] }],
    mahalle:   [{ ad:"AD", label:"Mahalle Adı", tip:"text" }, { ad:"nufus", label:"Nüfus", tip:"number" }],
    yol:       [{ ad:"yol_adi", label:"Yol Adı", tip:"text" }, { ad:"yol_tipi", label:"Yol Tipi", tip:"select",
                  secenekler:["Bulvar","Cadde","Sokak","Ara Yol"] }, { ad:"uzunluk_m", label:"Uzunluk (m)", tip:"number" }],
    numarataj: [{ ad:"kapi_no", label:"Kapı No", tip:"text" }, { ad:"daire_no", label:"Daire No", tip:"text" },
                { ad:"adres", label:"Adres", tip:"text" }],
  };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000b", display:"flex",
      ...
      <div style={{ background:"#1e2030", border:`1px solid ${katman.color}44`,
        borderRadius:14, padding:24, width:380, boxShadow:`0 0 32px ${katman.color}22` }}>
        ...
        {alanlar.map(alan => (
          <div key={alan.ad} style={{ marginBottom:12 }}>
            <label style={{ fontSize:11, color:"#7a80a0", display:"block", marginBottom:5,
              textTransform:"uppercase", letterSpacing:"0.05em" }}>{alan.label}</label>
            {alan.tip === "select" ? (
              ...
              <input type={alan.tip} placeholder={alan.label + " girin"}
                value={degerler[alan.ad] || ""} onChange={e => setDegerler(d => ({ ...d, [alan.ad]:e.target.value }))} style={sf.input} />
            )}
          </div>
        ))}
        <div style={{ marginBottom:16 }}>
          ...
          <textarea placeholder="Ek açıklama..." value={degerler.not || ""}
            onChange={e => setDegerler(d => ({ ...d, not:e.target.value }))}
            style={{ ...sf.input, height:60, resize:"vertical" }} />
        </div>
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          <button onClick={onIptal} style={{ padding:"8px 16px", background:"transparent",
            border:"1px solid #3a3f55", borderRadius:7, color:"#7a80a0", cursor:"pointer", fontFamily:"inherit", fontSize:13 }}>
            İptal
          </button>
          <button onClick={() => onKaydet(degerler)} style={{ padding:"8px 16px", background:katman.color,
            border:"none", borderRadius:7, color:"#fff", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:600 }}>
            ✓ Kaydet
          </button>
        </div>
      </div>
...
// Her katmanın kaç özelliği olduğunu say
const exportKatmanlar = [
  ...
  })),
  {
    ...
    count: (drawSrcRef.current?.getFeatures?.()?.length ?? 0) +
      Object.values(katmanSrcRef?.current || {}).reduce((s, src) => s + (src?.getFeatures?.()?.length ?? 0), 0),
  },
  {
    v: "import", l: "İçe Aktarılan", color: "#f59e0b",
    count: layersRef.current.__import?.getSource?.()?.getFeatures?.()?.length ?? 0,
  },
  {
    v: "sikayet", l: "Şikayetler", color: "#ef4444",
    count: layersRef.current.__sikayet?.getSource?.()?.getFeatures?.()?.length ?? 0,
  },
];
...
const [exportSecili, setExportSecili] = useState(exportKatmanlar.map(k => k.v));
  ...
  // Seçili her katman için ayrı dosya indir
  exportSecili.forEach(katmanKey => {
    let features = [];

    if (katmanKey === "cizim") {
      features.push(...(drawSrcRef.current?.getFeatures() || []));
      Object.values(katmanSrcRef?.current || {}).forEach(src => {
        if (src?.getFeatures) features.push(...src.getFeatures());
      });
    } else if (katmanKey === "import") {
      const src = layersRef.current.__import?.getSource?.();
      features.push(...(src?.getFeatures?.() || []));
    } else if (katmanKey === "sikayet") {
      const src = layersRef.current.__sikayet?.getSource?.();
      features.push(...(src?.getFeatures?.() || []));
    } else {
      // GeoJSON katmanı (parsel, yapi, mahalle, yol, numarataj)
      const src = layersRef.current[katmanKey]?.getSource?.();
      features.push(...(src?.getFeatures?.() || []));
    }

    if (features.length === 0) return; // Boş katmanı atla

    const katmanMeta = exportKatmanlar.find(k => k.v === katmanKey);
    const geojson = {
      type: "FeatureCollection",
      crs: { type: "name", properties: { name: "EPSG:4326" } },
      katman: katmanKey,
      katman_adi: katmanMeta?.l || katmanKey,
      tarih: new Date().toISOString(),
      kayit_sayisi: features.length,
    ...
    a.href     = url;
    a.download = `yetkin_cbs_${katmanKey}_${Date.now()}.geojson`;
    a.click();
  ...
  function handleFile(e) {
    ...
    reader.onload = evt => {
      try {
        ...
        setPendingGJ(parsed);
      } catch { setValidation({ valid:false, errors:["Dosya parse edilemedi — geçerli JSON değil."], warnings:[], count:0 }); }
  ...
  if (pendingGJ && importHedef) {
    onImport(pendingGJ, importHedef);
    onClose();
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000a", display:"flex",
      ...
      <div style={{ background:"#1a1d27", border:"1px solid #2a2f45", borderRadius:16,
        padding:24, minWidth:400, maxWidth:520, width:"90vw", maxHeight:"80vh", overflowY:"auto" }}>
        ...
        <div style={{ display:"flex", gap:4, marginBottom:20, background:"#0f1117", borderRadius:8, padding:4 }}>
          {["import","export"].map(t => (
            <button key={t} onClick={() => { setAktif(t); setValidation(null); setPendingGJ(null); }}
              style={{ flex:1, padding:8, borderRadius:6, border:"none", cursor:"pointer",
                fontFamily:"inherit", fontSize:13,
                background:aktif === t ? "#3b82f6" : "transparent",
                color:aktif === t ? "white" : "#7a80a0" }}>
        ...
        {aktif === "import" && (
          <>
            {!validation ? (
              <>
                <label htmlFor="gj-file" style={{ display:"block", border:"2px dashed #2a2f45",
                  borderRadius:10, padding:24, textAlign:"center", cursor:"pointer", marginBottom:12 }}
                  ...
                  <div style={{ fontSize:11, color:"#4a5070", marginTop:4 }}>.geojson, .json</div>
                </label>
                <input id="gj-file" type="file" accept=".json,.geojson" style={{ display:"none" }} onChange={handleFile} />
                {importing && <div style={{ color:"#7a80a0", fontSize:12, textAlign:"center" }}>🔍 Dosya analiz ediliyor...</div>}
              </>
            ) : (
              <div>
                <div style={{ background:validation.valid ? "#0f2a1a" : "#2a0f0f",
                  border:`1px solid ${validation.valid ? "#22c55e" : "#ef4444"}44`,
                  borderRadius:10, padding:14, marginBottom:12 }}>
                  <div style={{ fontSize:13, fontWeight:600, color:validation.valid ? "#22c55e" : "#ef4444", marginBottom:8 }}>
                    {validation.valid ? "✅ Dosya Geçerli" : "❌ Doğrulama Hatası"}
                  </div>
                  <div style={{ fontSize:12, color:"#94a3b8", marginBottom:8 }}>
                    <strong>{validation.count}</strong> özellik bulundu
                    {validation.geomTypes?.length > 0 && ` · ${validation.geomTypes.join(", ")}`}
                  </div>
                  {validation.errors.length > 0 && (
                    <div style={{ marginBottom:8 }}>
                      {validation.errors.map((e,i) => (
                        <div key={i} style={{ fontSize:12, color:"#f87171", padding:"3px 0",
                          borderLeft:"2px solid #ef4444", paddingLeft:8, marginBottom:4 }}>⚠ {e}</div>
                      ))}
                    </div>
                  )}
                    ...
                    <div>
                      {validation.warnings.map((w,i) => (
                        <div key={i} style={{ fontSize:12, color:"#fbbf24", padding:"3px 0",
                          borderLeft:"2px solid #f59e0b", paddingLeft:8, marginBottom:4 }}>ℹ {w}</div>
                      ))}
                    </div>
                  )}
                  {validation.propKeys?.length > 0 && (
                    <div style={{ marginTop:8, fontSize:11, color:"#64748b" }}>
                      Alanlar: {validation.propKeys.slice(0,8).join(", ")}{validation.propKeys.length > 8 ? "..." : ""}
                    </div>
                  )}
                </div>
                {/* Katman seçimi */}
{validation.count > 0 && (
  <div style={{ marginBottom:12 }}>
    <div style={{ fontSize:11, color:"#7a80a0", marginBottom:8,
      textTransform:"uppercase", letterSpacing:"0.05em" }}>
      Hangi katmana aktarılsın?
    </div>
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
      {[
...
<div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
  <button onClick={() => { setValidation(null); setPendingGJ(null); }}
    style={{ padding:"8px 14px", background:"transparent", border:"1px solid #3a3f55",
      borderRadius:8, color:"#7a80a0", cursor:"pointer", fontFamily:"inherit", fontSize:13 }}>
    Başka Dosya
  </button>
  {validation.count > 0 && (
    <button onClick={confirmImport}
      style={{ padding:"8px 14px", border:"none", borderRadius:8, cursor:"pointer",
        ...
        background: validation.valid ? "#3b82f6" : "#f59e0b",
        opacity: !importHedef ? 0.4 : 1 }}
      disabled={!importHedef}>
      {!importHedef
        ? "Katman Seç"
        : validation.valid
          ? `✓ ${LAYER_DEFS.find(d=>d.key===importHedef)?.label || "İçe Aktarılan"}'a Aktar`
          : "⚠ Yine de Aktar"}
    </button>
  )}
</div>
        ...
        {aktif === "export" && (
  <>
    <div style={{ fontSize:12, color:"#7a80a0", marginBottom:10 }}>
      Hangi katmanları indirmek istiyorsun?
    </div>

    {/* Tümünü seç */}
    <label style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px",
      borderRadius:8, cursor:"pointer", marginBottom:8, fontSize:12,
      background:"#1a1d27", border:"1px solid #2a2f45" }}>
      <input type="checkbox"
        checked={exportSecili.length === exportKatmanlar.length}
        onChange={e => setExportSecili(e.target.checked ? exportKatmanlar.map(k => k.v) : [])} />
      <span style={{ color:"#e8eaf0", fontWeight:600 }}>Tümünü Seç</span>
      <span style={{ marginLeft:"auto", fontSize:10, color:"#4a5070" }}>
        {exportSecili.length}/{exportKatmanlar.length} seçili
      </span>
    </label>

    {/* Her katman ayrı satır */}
    {exportKatmanlar.map(({ v, l, color, count }) => (
      <label key={v} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px",
        borderRadius:8, cursor:"pointer", marginBottom:4, fontSize:12,
        ...
        <input type="checkbox"
          checked={exportSecili.includes(v)}
          onChange={e => setExportSecili(s =>
            e.target.checked ? [...s, v] : s.filter(x => x !== v)
          )} />
    ...
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:16, gap:8 }}>
      <span style={{ fontSize:11, color:"#4a5070" }}>
        {exportSecili.length === 0
          ? "En az bir katman seç"
          : exportSecili.length === 1
            ? "1 katman indirilecek"
            : `${exportSecili.length} katman — ayrı ayrı indirilecek`}
      </span>
      <div style={{ display:"flex", gap:8 }}>
        <button style={ms.ghost} onClick={onClose}>İptal</button>
        <button style={{ ...ms.primary, opacity: exportSecili.length === 0 ? 0.4 : 1 }}
          disabled={exportSecili.length === 0}
          onClick={handleExport}>
          ⬇ {exportSecili.length > 1 ? `${exportSecili.length} Dosya İndir` : "GeoJSON İndir"}
        </button>
      </div>
    </div>
  </>
...
const ms = {
  primary: { padding:"9px 16px", background:"#3b82f6", color:"white",
    border:"none", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:13 },
  ghost: { padding:"9px 16px", background:"transparent", color:"#7a80a0",
    border:"1px solid #2a2f45", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:13 },
};

// ============================================================================
// KATMAN SIRALAMA MODALİ
// ============================================================================
function LayerOrderModal({ layerOrder, setLayerOrder, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000a", display:"flex",
      alignItems:"center", justifyContent:"center", zIndex:250, backdropFilter:"blur(4px)" }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background:"#1a1d27", border:"1px solid #2a2f45", borderRadius:16, padding:24, width:360 }}>
...
function StatisticsPanel({ katmanSrcRef, drawSrcRef, sikayetSrcRef, onClose }) {
  ...
  useEffect(() => {
    ...
    LAYER_DEFS.forEach(def => {
      const src = katmanSrcRef.current?.[def.key];
      if (!src) return;
      ...
      features.forEach(f => {
        ...
        try {
          ...
          if (type === "Polygon" || type === "MultiPolygon") {
            ...
            totalArea += Math.abs((ext[2]-ext[0]) * (ext[3]-ext[1]));
          }
      ...
      result.push({
        label: def.label, color: def.color, count: features.length,
        area: totalArea > 0 ? (totalArea / 10000).toFixed(2) + " ha" : null,
        length: totalLength > 0 ? (totalLength / 1000).toFixed(2) + " km" : null,
      });
    });

    const drawFeatures = drawSrcRef.current?.getFeatures() || [];
    if (drawFeatures.length > 0) {
      result.push({ label:"Çizimler", color:"#ff6600", count:drawFeatures.length, area:null, length:null });
    }

    const sikayetFeatures = sikayetSrcRef.current?.getFeatures() || [];
    if (sikayetFeatures.length > 0) {
      const byDurum = { beklemede:0, incelemede:0, cozuldu:0 };
      sikayetFeatures.forEach(f => { const d = f.get("durum") || "beklemede"; byDurum[d] = (byDurum[d]||0)+1; });
      result.push({ label:"Şikayetler", color:"#ef4444", count:sikayetFeatures.length,
        extra:`Bekleyen:${byDurum.beklemede} · İncelenen:${byDurum.incelemede} · Çözülen:${byDurum.cozuldu}`, area:null, length:null });
    }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000a", display:"flex",
      alignItems:"center", justifyContent:"center", zIndex:250, backdropFilter:"blur(4px)" }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background:"#1a1d27", border:"1px solid #2a2f45", borderRadius:16, padding:24,
        ...
        {stats.length === 0 ? (
          <div style={{ color:"#7a80a0", fontSize:13, textAlign:"center", padding:24 }}>
            Henüz hiç çizim verisi yok.
          </div>
        ) : stats.map((s, i) => (
          <div key={i} style={{ background:"#0f1117", border:`1px solid ${s.color}33`,
            ...
            {(s.area || s.length) && (
              <div style={{ display:"flex", gap:12, fontSize:12, color:"#64748b" }}>
                {s.area   && <span>Alan: <strong style={{color:"#94a3b8"}}>{s.area}</strong></span>}
                {s.length && <span>Uzunluk: <strong style={{color:"#94a3b8"}}>{s.length}</strong></span>}
              </div>
            )}
...
function MapApp({ kullanici, onCikis }) {
  ...
  // ── DÜZELTİLDİ: interaction ref'leri daha güvenli yönetmek için ──────────
  const drawInterRef   = useRef(null);
  const modifyInterRef = useRef(null);
  ...
  const [layerVisibility,  setLayerVis]          = useState({
    ...
    poi:true, cizim:true, import:true, sikayet:true,
  });
  ...
  const defaultOrder = [...LAYER_DEFS.map(d => d.key), "poi", "sikayet", "cizim", "import"];
  ...
  useEffect(() => {
    ...
    const map = new Map({
      ...
      layers: [osmLayer, uyduLayer, drawLayer, importLayer, sikayetLayer, routeLayer, hlLayer],
    ...
    setTimeout(() => {
      loadCizimler(katmanSrcRef, drawSrcRef, setDrawCount);
      loadSikayetler(sikayetSrcRef);
    }, 200);
    ...
    map.on("click", evt => {
      if (sikayetModeRef.current) {
        ...
        setSikayetKonum(lonLat);
        setSikayetFormAcik(true);
        return;
      }
    ...
    return () => {
      ...
      // ── DÜZELTİLDİ: interaction'ları önce temizle, sonra haritayı yok et ──
      const currentMap = mapInstanceRef.current;
      if (currentMap) {
        [drawInterRef, modifyInterRef, snapInterRef].forEach(ref => {
          if (ref.current) {
            try { currentMap.removeInteraction(ref.current); } catch {}
            ref.current = null;
          }
        });
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        const features = new GeoJSON().readFeatures(data, {
          dataProjection: USE_GEOSERVER ? MAP_PROJECTION : DATA_PROJECTION,
          featureProjection: MAP_PROJECTION,
        });
        const layer = new VectorLayer({
          source: new VectorSource({ features }),
          style:  makeFilterStyle(styleFor(def.key, def.color), def.key),
          visible: visibilityRef.current[def.key] !== false,
        });
        map.addLayer(layer);
  ...
  const loadPoiLayer = (mapInst) => {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        const layer = new VectorLayer({
          source: new VectorSource({ features }), style:makeFilterStyle(poiStyle,"poi"),
          zIndex:500, visible:visibilityRef.current.poi !== false,
        });
  ...
  const makeFilterStyle = (baseStyle, key) => (feature, resolution) => {
    if (mahalleFilterRef.current && key !== "mahalle") {
      if (!isInsideMahalle(feature, mahalleFilterRef.current)) return null;
    }
    if (key === "poi") {
      const kat = (feature.get("kategori") || "diger").toLowerCase();
      if (!activeCatRef.current[kat]) return null;
    }
  ...
  useEffect(() => {
    ...
    base.osm.setVisible(baseMap === "osm");
    base.uydu.setVisible(baseMap === "uydu");
  }, [baseMap]);

  // ── Katman görünürlüğü ────────────────────────────────────────────────────
  useEffect(() => {
    visibilityRef.current = layerVisibility;
    Object.entries(layerVisibility).forEach(([key, vis]) => {
      const layer = layersRef.current[key]; if (layer?.setVisible) layer.setVisible(vis);
    });
  ...
  useEffect(() => {
    ...
    layersRef.current.poi?.changed();
  }, [activeCategories]);

  // ── Şikayet modu ref sync ─────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      fetch("https://nominatim.openstreetmap.org/search?" + new URLSearchParams({
        q:addressQuery, format:"json", addressdetails:"1", limit:"8",
        countrycodes:"tr", viewbox:"40.7,38.4,41.3,37.9", bounded:"0",
  ...
  const goToAddress = (item) => {
    ...
    f.setProperties({ __address:true, Adres:item.display_name,
      Enlem:parseFloat(item.lat).toFixed(6), Boylam:parseFloat(item.lon).toFixed(6) });
    hl.addFeature(f);
    setSelectedFeature({ Adres:item.display_name,
      Enlem:parseFloat(item.lat).toFixed(6), Boylam:parseFloat(item.lon).toFixed(6) });
  ...
  // ── DÜZELTİLDİ: applyDrawInteraction — güvenli interaction yönetimi ──────
  const applyDrawInteraction = useCallback((tool, katman) => {
    const map = mapInstanceRef.current;
    if (!map) return;

    // Önce mevcut interaction'ları güvenli şekilde kaldır
    const removeInteraction = (ref) => {
      if (ref.current) {
        try {
          map.removeInteraction(ref.current);
        } catch (e) {
          // Zaten kaldırılmış veya harita yok — sessizce devam et
        }
        ref.current = null;
      }
    };

    removeInteraction(snapInterRef);
    removeInteraction(modifyInterRef);
    removeInteraction(drawInterRef);
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      try {
        hedefSrc.removeFeature(feature);
      } catch (e) {}
...
map.addInteraction(draw);
      drawInterRef.current = draw;

      const modify = new Modify({ source: hedefSrc });
      map.addInteraction(modify);
      modifyInterRef.current = modify;

      const snap = new Snap({ source: hedefSrc });
      map.addInteraction(snap);
      snapInterRef.current = snap;
    } catch (e) {
      console.warn("Interaction eklenirken hata:", e);
    }
  }, []);

  const setDrawTool = (val) => {
    drawToolRef.current = val;
    setDrawToolState(val);
    applyDrawInteraction(val, cizimKatmanRef.current);
  };

  const setCizimKatman = (val) => {
    cizimKatmanRef.current = val;
    setCizimKatmanState(val);
    if (drawToolRef.current && drawToolRef.current !== "erase")
      applyDrawInteraction(drawToolRef.current, val);
  };
  ...
  try {
    const features = new GeoJSON().readFeatures(geojson, {
      dataProjection: "EPSG:4326", featureProjection: MAP_PROJECTION,
    });

    if (features.length === 0) { alert("Dosyada özellik bulunamadı."); return; }

    // Hedef katmana göre doğru source'u seç
    let hedefSrc = null;
    let hedefAdi = "İçe Aktarılan";

    if (hedefKatman === "import") {
      hedefSrc = importSrcRef.current;
      hedefAdi = "İçe Aktarılan";
    } else if (katmanSrcRef.current[hedefKatman]) {
      // Admin çizim katmanlarına aktar
      hedefSrc = katmanSrcRef.current[hedefKatman];
      hedefAdi = LAYER_DEFS.find(d => d.key === hedefKatman)?.label || hedefKatman;
    } else {
      // GeoJSON katmanının source'una doğrudan ekle
      hedefSrc = layersRef.current[hedefKatman]?.getSource?.();
      hedefAdi = LAYER_DEFS.find(d => d.key === hedefKatman)?.label || hedefKatman;
    }

    if (!hedefSrc) { alert("Hedef katman bulunamadı."); return; }

    // Her feature'a hangi katmandan geldiğini işaretle
    ...
    if (extent && extent[0] !== Infinity)
      mapInstanceRef.current?.getView().fit(extent, { duration:800, maxZoom:18, padding:[40,40,40,40] });
    ...
    <div style={{ display:"flex", height:"100vh", width:"100vw", overflow:"hidden",
      ...
      <div style={ss.left}>
        ...
        <div style={{ marginBottom:8 }}>
          ...
          <div style={{ background:"#333", borderRadius:6, padding:"6px 8px",
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              <button onClick={onCikis}
                style={{ background:"transparent", border:"1px solid #555",
                  borderRadius:4, color:"#aaa", fontSize:11, cursor:"pointer", padding:"3px 6px" }}>
                Çıkış
              </button>
              {/* ── DÜZELTİLDİ: Şifre butonu sadece canEdit kullanıcılara göster ── */}
              {canEdit && (
                <button onClick={() => setShowSifreDegistir(true)}
                  style={{ background:"transparent", border:"1px solid #3b82f655",
                    borderRadius:4, color:"#3b82f6", fontSize:10, cursor:"pointer", padding:"2px 6px" }}>
                  🔑 Şifre
                </button>
              )}
              {isSuperAdmin && (
                <button onClick={() => setShowKullaniciYon(true)}
                  style={{ background:"transparent", border:"1px solid #a855f755",
                    borderRadius:4, color:"#a855f7", fontSize:10, cursor:"pointer", padding:"2px 6px" }}>
                  👥 Kullanıcılar
                </button>
              )}
            </div>
        ...
        <button
          onClick={() => {
            ...
            else { setSikayetMod(true); setRotaMod(false); temizleRota(); setDrawTool(null); }
          }}
          style={{ ...ss.btn, width:"100%", textAlign:"center", marginBottom:6,
            background: sikayetMod ? "#ef444422" : "#444",
            border: sikayetMod ? "1px solid #ef4444" : "1px solid #555",
            color: sikayetMod ? "#ef4444" : "#ccc" }}>
          🚨 {sikayetMod ? "Şikayet Modu Aktif — İptal" : "Şikayet / Talep Bildir"}
        </button>
        {sikayetMod && (
          <div style={{ fontSize:11, color:"#f59e0b", padding:"5px 8px", background:"#2a1f0a",
            borderRadius:6, marginBottom:6, border:"1px solid #f59e0b44" }}>
        ...
        <button
          onClick={() => {
            if (rotaMod) { temizleRota(); }
              ...
              setRotaMod(true); setSikayetMod(false);
              routeMode.current = "start"; setRotaAdim("start"); setDrawTool(null);
            }
          }}
          style={{ ...ss.btn, width:"100%", textAlign:"center", marginBottom:6,
        ...
        {rotaMod && (
          <div style={{ background:"#0f1729", border:"1px solid #3b82f644", borderRadius:8,
            ...
            {rotaBilgi && (
              <div style={{ color:"#e8eaf0" }}>
                ...
                <button onClick={() => { temizleRota(); setRotaMod(true); routeMode.current="start"; setRotaAdim("start"); }}
                  style={{ marginTop:6, padding:"4px 10px", background:"#3b82f633", border:"1px solid #3b82f6",
                    borderRadius:6, color:"#3b82f6", cursor:"pointer", fontFamily:"inherit", fontSize:11 }}>
                  ↺ Yeni Rota
                </button>
              </div>
            )}
        ...
        {searchResults.length > 0 && (
          <div style={ss.results}>
            {searchResults.map((item, idx) => (
              <div key={idx} onClick={() => zoomToFeature(item.feature)} style={ss.resultItem}
        ...
        {addressResults.length > 0 && (
          <div style={ss.results}>
            {addressResults.map((item, idx) => (
              <div key={item.place_id||idx} onClick={() => goToAddress(item)} style={ss.resultItem}
                onMouseEnter={e => e.currentTarget.style.background="#555"}
                onMouseLeave={e => e.currentTarget.style.background="transparent"}>
          ...
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:2 }}>
              {POI_CATEGORIES.map(cat => (
                <label key={cat.key} style={{ ...ss.layerRow, fontSize:11, padding:"3px 4px",
                  background:activeCategories[cat.key] ? "#3a3a3a" : "transparent", borderRadius:3 }}>
                  ...
                  <span style={{ display:"inline-block", width:14, height:14, borderRadius:"50%",
                    background:cat.color, color:"#fff", fontSize:9,
                    textAlign:"center", lineHeight:"14px", fontWeight:"bold" }}>{cat.symbol}</span>
                  <span>{cat.label}</span>
                </label>
              ))}
            </div>
        ...
        {canEdit && (
          <>
            ...
            <div style={{ display:"flex", gap:4, marginBottom:8 }}>
              <button onClick={() => { undoRedo.undo(); setTimeout(() => saveCizimler(katmanSrcRef, drawSrcRef), 50); }}
                style={{ flex:1, padding:"6px 4px", fontSize:11, borderRadius:6, cursor:"pointer",
                  fontFamily:"inherit", border:"1px solid #555", background:"#444", color:"#ccc" }}>
                ↩ Geri Al <span style={{ fontSize:9, color:"#888" }}>Ctrl+Z</span>
              </button>
              <button onClick={() => { undoRedo.redo(); setTimeout(() => saveCizimler(katmanSrcRef, drawSrcRef), 50); }}
                style={{ flex:1, padding:"6px 4px", fontSize:11, borderRadius:6, cursor:"pointer",
                  fontFamily:"inherit", border:"1px solid #555", background:"#444", color:"#ccc" }}>
                ↪ İleri Al <span style={{ fontSize:9, color:"#888" }}>Ctrl+Y</span>
              </button>
            </div>

            <div style={{ fontSize:10, color:"#7a80a0", marginBottom:4, textTransform:"uppercase", letterSpacing:"0.05em" }}>
              1. Katman Seç
            </div>
            <div style={{ marginBottom:10 }}>
              {LAYER_DEFS.map(def => (
              ...
              {drawTools.map(({ id, label }) => {
                ...
                return (
                  <button key={id} onClick={() => setDrawTool(drawTool === id ? null : id)}
                    style={{ padding:"6px 4px", fontSize:11, textAlign:"center", borderRadius:6,
            ...
            {drawTool === "erase" && (
              <div style={{ fontSize:11, color:"#ff6666", padding:"5px 8px",
                background:"#3a1a1a", borderRadius:6, marginBottom:8 }}>
                Silmek istediğin çizime tıkla
              </div>
            )}

            <div style={{ fontSize:11, color:"#555", marginBottom:8 }}>
              Toplam {drawCount} çizim objesi ·{" "}
              <span style={{color:"#22c55e", fontSize:10}}>💾 Otomatik kaydediliyor</span>
            </div>
            ...
            <button onClick={() => setShowModal(true)}
              style={{ ...ss.btn, background:"#1976d2", borderColor:"#1976d2", color:"white", width:"100%", marginBottom:4 }}>
              📦 GeoJSON Import / Export
            </button>
            <button onClick={() => setShowLayerOrder(true)}
              style={{ ...ss.btn, background:"#374151", borderColor:"#555", color:"#ccc", width:"100%", marginBottom:4 }}>
              🗂 Katman Sıralama
            </button>
            <button onClick={() => setShowStats(true)}
              style={{ ...ss.btn, background:"#1a2a1a", borderColor:"#22c55e88", color:"#22c55e", width:"100%", marginBottom:4 }}>
        ...
        <div style={{ position:"absolute", bottom:0, left:0, right:0, height:26,
          ...
          {rotaMod && rotaAdim && (
            <span style={{ color:"#3b82f6" }}>
              🗺 Rota: {rotaAdim==="start"?"Başlangıç seç":rotaAdim==="end"?"Bitiş seç":"Hesaplanıyor"}
            </span>
          )}
          {drawTool && !sikayetMod && !rotaMod && (
            <span style={{ color:"#ff6600" }}>✏ {drawTool} modu aktif</span>
          )}
              ...
              <div style={{ marginTop:10 }}>
                ...
                {["beklemede","incelemede","cozuldu"].map(d => (
                  <button key={d} onClick={() => {
                    ...
                    style={{ marginRight:4, marginBottom:4, padding:"4px 10px", borderRadius:6, cursor:"pointer",
          ...
          <>
            ...
            {(sikayetSrcRef.current?.getFeatures()?.length ?? 0) === 0 ? (
              ...
              sikayetSrcRef.current?.getFeatures().slice(-5).reverse().map((f, i) => {
                ...
                return (
                  <div key={i} style={{ background:"#2a2a2a", borderRadius:6, padding:"6px 8px", marginBottom:4,
                    borderLeft:`3px solid ${kat.color}`, cursor:"pointer" }}
                    onClick={() => {
      ...
      {/* ── MODALLAR ─────────────────────────────────────────────────────────── */}
      {ozellikForm && (
        <OzellikFormModal ozellikForm={ozellikForm} katmanDefs={LAYER_DEFS}
          onKaydet={handleOzellikKaydet} onIptal={handleOzellikIptal} />
      )}
      {sikayetFormAcik && (
        <SikayetFormModal konum={sikayetKonum}
          onKaydet={handleSikayetKaydet}
          onIptal={() => { setSikayetFormAcik(false); setSikayetKonum(null); }} />
      )}
      {showModal && (
        <ImportExportModal onClose={() => setShowModal(false)} onImport={handleImport}
          layersRef={layersRef} drawSrcRef={drawSrcRef} katmanSrcRef={katmanSrcRef} />
      )}
      {showLayerOrder && (
        <LayerOrderModal layerOrder={layerOrder} setLayerOrder={setLayerOrder}
          onClose={() => setShowLayerOrder(false)} />
      )}
      {showStats && (
        <StatisticsPanel katmanSrcRef={katmanSrcRef} drawSrcRef={drawSrcRef}
          sikayetSrcRef={sikayetSrcRef} onClose={() => setShowStats(false)} />
      )}
      {showSifreDegistir && (
        <SifreDegistirModal kullanici={kullanici} onKapat={() => setShowSifreDegistir(false)} />
      )}
      {showKullaniciYon && (
        <KullaniciYonetimModal onKapat={() => setShowKullaniciYon(false)} />
      )}
    </div>
  );
}
...
const ss = {
  left: { width:265, minWidth:265, background:"#2c2c2c", color:"white",
    padding:12, display:"flex", flexDirection:"column", gap:4, overflowY:"auto" },
  ...
  input: { width:"100%", padding:8, borderRadius:4, border:"1px solid #555",
    background:"#444", color:"white", fontSize:12, boxSizing:"border-box" },
  ...
  btn: { padding:"6px 10px", background:"#444", color:"white", border:"1px solid #555",
    borderRadius:4, cursor:"pointer", fontSize:12, marginBottom:4, textAlign:"left", width:"100%" },
